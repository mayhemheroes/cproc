...
./configure CC="$CC" CFLAGS="$COMMON_CFLAGS -O2 $DEBUG_FLAGS"
...
make clean >/dev/null 2>&1 || true
rm -f config.h config.mk
./configure CC="$CC" CFLAGS="$COMMON_CFLAGS $SANITIZER_FLAGS $DEBUG_FLAGS" LDFLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS"
make -j"$MAYHEM_JOBS" cproc-qbe
...
EOF
  ...
  $CC $SANITIZER_FLAGS $DEBUG_FLAGS -o cproc-qbe \
